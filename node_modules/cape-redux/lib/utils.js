...
var dispatcher = exports.dispatcher = function dispatcher(dispatch) {
  return function (action) {
    return (0, _flow3.default)(action, dispatch);
  };
};
...
function mapDispatchToProps(getActions) {
  return function (dispatch, props) {
    return _extends({ dispatch: dispatch }, (0, _redux.bindActionCreators)(getActions(props), dispatch));
  };
}
...
function createMiddleware(actions) {
  return function (store) {
    return function (next) {
      return function (action) {
        if (!action.type) return next(action);
        if ((0, _isFunction3.default)(actions[action.type])) {
          return actions[action.type]({ action: action, next: next, store: store });
        }
        return next(action);
      };
    };
